...
输入**题目 + 课堂笔记 + 你的答案**，让多个不同厂商的大模型（DeepSeek / 豆包 / OpenAI…）作为独立评审**并发流式**地按"课堂笔记要点覆盖度"打分、列出漏点，最后由一位"议长" Agent 合议裁定，归并成按优先级排序的补漏清单。你据此改答案、二次提交，评审带上一轮上下文继续评。
...
- **简洁**（默认）：零 WebGL。状态条 + 每评审一张结果卡 + 合议裁定。功能完整，导出、历史、逐要点证据都在。
...
3. 读合议裁定的补漏清单 → 改答案 → 点「带上下文重新评分」。

## 功能

- **导出报告**：一键复制到剪贴板并下载 `.md`，含题目/笔记/答案、各评审打分与点评、合议裁定、补漏清单、分歧要点。
